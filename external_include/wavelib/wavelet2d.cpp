...
double convfftm_OMP(vector<double> &a, vector<double> &b, vector<double> &c) {
	fftw_complex *inp_data, *filt_data, *inp_fft, *filt_fft, *temp_data, *temp_ifft;
...
#pragma omp parallel for
	...
	std::vector<double> c_tmp(sz);
#pragma omp parallel for
	for (int i = 0; i < sz; i++) {
		double temp1;
		c_tmp[i] = temp_ifft[i][0] / (double)sz;
	}
...
void downsamp_OMP(vector<double> &sig, int M, vector<double> &sig_d) {
	int len = sig.size();
	double len_n = ceil((double)len / (double)M);
	vector<double>sig_d_tmp(len_n);
#pragma omp parallel for
	for (int i = 0; i < (int)len_n; i++) {
		sig_d_tmp[i] = sig[i*M];
	}
	sig_d.insert(sig_d.end(), sig_d_tmp.begin(), sig_d_tmp.end());
}
...
//		int len_cA =(int)  floor(double (len_sig + len_lpfilt -1) / double (2));
	vector<double> cA_undec;
	// convolving signal with lpd, Low Pass Filter, and O/P is stored in cA_undec
	convfftm_OMP(signal, lpd, cA_undec);
	int D = 2; // Downsampling Factor is 2
	cA_undec.erase(cA_undec.begin(), cA_undec.begin() + len_avg - 1);
	cA_undec.erase(cA_undec.end() - len_avg + 1, cA_undec.end());
	cA_undec.erase(cA_undec.begin() + len_sig, cA_undec.end());
	...
	// Downsampling by 2 gives cA
	downsamp_OMP(cA_undec, D, cA);//【被我注释掉】
...
//		int len_cA =(int)  floor(double (len_sig + len_lpfilt -1) / double (2));
	...
	vector<double> cD_undec;
	// convolving signal with lpd, Low Pass Filter, and O/P is stored in cA_undec
	convfftm_OMP(signal, hpd, cD_undec);
	cD_undec.erase(cD_undec.begin(), cD_undec.begin() + len_avg - 1);
	cD_undec.erase(cD_undec.end() - len_avg + 1, cD_undec.end());
	cD_undec.erase(cD_undec.begin() + len_sig, cD_undec.end());
	cD_undec.erase(cD_undec.begin());
...
void* dwt_2d_OMP(vector<vector<double> > &origsig, int J, string nm, vector<double> &dwt_output
	, vector<double> &flag, vector<int> &length) {
	// flag will contain
	...
	for (int iter = 0; iter < J; iter++) {
		...
		if (iter == 0) {
			dwt2_OMP(nm, original_copy, cA, cH, cV, cD);
		}
		else {
			dwt2_OMP(nm, original_copy, cA, cH, cV, cD);
...
void* dwt2_OMP(string name, vector<vector<double> > &signal, vector<vector<double> >  &cLL,
	vector<vector<double> >  &cLH, vector<vector<double> >  &cHL, vector<vector<double> > &cHH) {
	//Analysis
	int rows = signal.size();
	int cols = signal[0].size();
	...
	// Implementing row filtering and column downsampling in each branch.
//#pragma omp parallel for//【被我注释掉】+
//	for (int i = 0; i < rows; i++) {
//		vector<double> oup_lp, oup_hp;
//		vector<double> temp_row(cols);
//		for (int j = 0; j < cols; j++) {
//			double temp = signal[i][j];
//			temp_row[j] = temp;
//		}
//		dwt1_m_OMP(name, temp_row, oup_lp, oup_hp);
//		for (int j = 0; j < (int)oup_lp.size(); j++) {
//			lp_dn1[i][j] = oup_lp[j];
//			hp_dn1[i][j] = oup_hp[j];
//		}
//	}//+【被我注释掉】
	...
	//for (int j = 0; j < cols; j++) {
	//	vector<double> oup_lp, oup_hp;
	//	vector<double> temp_row3(rows);
	//	for (int i = 0; i < rows; i++) {
	//		double temp = lp_dn1[i][j];
	//		temp_row3[i]=temp;
	//	}
	//	dwt1_m_OMP(name, temp_row3, oup_lp, oup_hp);
	//	for (int i = 0; i < (int)oup_lp.size(); i++) {
	//		cLL[i][j] = oup_lp[i];
	//		cLH[i][j] = oup_hp[i];
	//	}
	...
	for (int j = 0; j < cols; j++) {
		vector<double> oup_lp, oup_hp;
		vector<double> temp_row5(rows);

		for (int i = 0; i < rows; i++) {
			double temp = hp_dn1[i][j];
			temp_row5[i]=temp;
		}
		dwt1_m_OMP(name, temp_row5, oup_lp, oup_hp);

		for (int i = 0; i < (int)oup_lp.size(); i++) {
			//cHL[i][j] = oup_lp[i];//【被我注释掉】
			cHH[i][j] = oup_hp[i];
		}
	}
	return 0;
