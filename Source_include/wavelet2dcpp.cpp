...
Wavelet2d::Wavelet2d()
{
	transient_size_of_fft = 0;
}

void* Wavelet2d::dwt_2d_OMP(vector<vector<double> > &origsig, int J, string nm, vector<double> &dwt_output
	...
	for (int iter = 0; iter < J; iter++) {
		...
		if (iter == 0) {
			this->dwt2_OMP(nm, original_copy, cA, cH, cV, cD);
		}
		else {
			this->dwt2_OMP(nm, original_copy, cA, cH, cV, cD);
		}
...
void* Wavelet2d::dwt2_OMP(string name, vector<vector<double> > &signal, vector<vector<double> >  &cLL,
	...
	vector<double> lp1, hp1, lp2, hp2;
	filtcoefs(name, lp1, hp1, lp2, hp2);
...
//#pragma omp parallel for//【被我注释掉】+
	for (int i = 0; i < rows; i++) {
		...
		this->dwt1_m_OMP(name, temp_row, oup_lp, oup_hp);
	...
	for (int j = 0; j < cols; j++) {
		...
		this->dwt1_m_OMP(name, temp_row3, oup_lp, oup_hp);
	...
	for (int j = 0; j < cols; j++) {
		...
		this->dwt1_m_OMP(name, temp_row5, oup_lp, oup_hp);

		for (int i = 0; i < (int)oup_hp.size(); i++) {
...
void* Wavelet2d::dwt1_m_OMP(string wname, vector<double> &signal, vector<double> &cA, vector<double> &cD) {

	vector<double> lpd, hpd, lpr, hpr;

	filtcoefs(wname, lpd, hpd, lpr, hpr);
...
//		int len_cA =(int)  floor(double (len_sig + len_lpfilt -1) / double (2));
	...
	this->downsamp_OMP(cA_undec, D, cA);//+【被我注释掉】
...
//		int len_cA =(int)  floor(double (len_sig + len_lpfilt -1) / double (2));
	...
	// convolving signal with lpd, Low Pass Filter, and O/P is stored in cA_undec
	this->convfftm_OMP(signal, hpd, cD_undec);
   ...
   // Downsampling by 2 gives cA
	this->downsamp_OMP(cD_undec, D, cD);
...
void Wavelet2d::downsamp_OMP(vector<double> &sig, int M, vector<double> &sig_d) {
	int len = sig.size();
...
void Wavelet2d::per_ext(vector<double>&sig , int a)
{
...
double Wavelet2d::convfftm_OMP(vector<double> &a, vector<double> &b, vector<double> &c) {
	...
	if (sz != transient_size_of_fft) {
		...
		if (transient_size_of_fft != 0) {
			fftw_destroy_plan(this->plan_forward_inp);
			fftw_destroy_plan(this->plan_forward_filt);
			fftw_destroy_plan(this->plan_backward);
		}
		//#pragma omp parallel

		this->plan_forward_inp = fftw_plan_dft_1d(sz, inp_data, inp_fft, FFTW_FORWARD, FFTW_MEASURE);
		this->plan_forward_filt = fftw_plan_dft_1d(sz, filt_data, filt_fft, FFTW_FORWARD, FFTW_MEASURE);
		this->plan_backward = fftw_plan_dft_1d(sz, temp_data, temp_ifft, FFTW_BACKWARD, FFTW_MEASURE);
	...
	fftw_execute_dft(this->plan_forward_inp, inp_data, inp_fft);
	fftw_execute_dft(this->plan_forward_filt, filt_data, filt_fft);
	...
	fftw_execute_dft(this->plan_backward, temp_data, temp_ifft);
