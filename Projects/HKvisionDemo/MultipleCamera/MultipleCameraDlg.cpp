...
unsigned int    __stdcall   autoStarting(void* pUser)
{
	if (NULL == pUser)
	{
		return -1;
	...
	pCam->OnBnClickedInitDeviceButton();
	Sleep(1000);
	pCam->OnBnClickedSetExpouseGainButton();
	Sleep(1000);
	pCam->SetFrameRate();
	pCam->OnBnClickedStartGrabbingButton();
...
unsigned int    __stdcall   WorkThread(void* pUser)
{
    ...
    pCam->ThreadFun(nUsingCameraNum, pCam);
...
BOOL CMultipleCameraDlg::OnInitDialog()
{
	...
	m_dExpouseEdit = 800.0;
	m_dGainEdit = 4.0;
	UpdateData(FALSE);

	unsigned int nThreadID = 0;
	void* phandle = (void*)_beginthreadex(NULL, 0, autoStarting, this, 0, &nThreadID);
	if (NULL == phandle)
	{
...
int CMultipleCameraDlg::SetFrameRate(void) {
	if (FALSE == m_bOpenDevice)
	{
		return STATUS_ERROR;
	}
	...
	for (i = 0; i < m_nUseNumEdit; i++)
	{
		...
		nRet = m_pcMyCamera[i]->SetFloatValue("AcquisitionFrameRate", 2.0);
	}
	return MV_OK;
}
...
int CMultipleCameraDlg::ThreadFun(int nUsingDeviceNum, void* pParent)
{
	...
	unsigned char* pDataForRGB = (unsigned char*)malloc(sizeof(unsigned char) * nDataSize*3);
	if (NULL == pDataForRGB)
	{
    ...
    while(m_bStartGrabbing)
    {
        ...
        if (nRet == MV_OK)
        {
			...
			//((CMultipleCameraDlg*)pParent)->trace_image(&stDisplayInfo, nUsingDeviceNum, "d://");
			...
			stConvertParam.pSrcData = pGrabBuf;                            //ch:输入数据缓存 | en:input data buffer
			...
			stConvertParam.nDstBufferSize = nDataSize*3;            //ch:输出缓存大小 | en:output buffer size
			nRet = m_pcMyCamera[nUsingDeviceNum]->ConvertPixelType(&stConvertParam);
			if (MV_OK != nRet)
			{
				printf("Convert Pixel Type fail! nRet [0x%x]\n", nRet);
				break;
			}
			stDisplayInfo.pData = pDataForRGB;
			stDisplayInfo.enPixelType = PixelType_Gvsp_RGB8_Packed;


			((CMultipleCameraDlg*)pParent)->trace_image(&stDisplayInfo, nUsingDeviceNum, "d://", pParent);
        }
        else
        {
...
void CMultipleCameraDlg::trace_image(MV_DISPLAY_FRAME_INFO* pFrameInfo, int nUsingDeviceNum, std::string save_filepath, void* pParent)
{
	cv::Mat srcImage;
	if (pFrameInfo->enPixelType == PixelType_Gvsp_Mono8)
	{
		srcImage = cv::Mat(pFrameInfo->nHeight, pFrameInfo->nWidth, CV_8UC1, pFrameInfo->pData);
	}
	else if (pFrameInfo->enPixelType == PixelType_Gvsp_RGB8_Packed)
	{
		//RGB2BGR(pData, pstImageInfo->nWidth, pstImageInfo->nHeight);
		srcImage = cv::Mat(pFrameInfo->nHeight, pFrameInfo->nWidth, CV_8UC3, pFrameInfo->pData);
	}
	else
	{
	...
	cv::Mat resizedMat;
	cv::resize(srcImage, resizedMat, cv::Size(), 0.5, 0.5);
	cv::Mat aChannels[3];
	cv::split(resizedMat, aChannels);
	cv::max(aChannels[0], aChannels[1], aChannels[0]);
	cv::max(aChannels[0], aChannels[2], aChannels[0]);
	cv::Mat grayMat = aChannels[0];

	//cv::cvtColor(resizedMat, resizedMat, cv::COLOR_BGR2GRAY);
	cv::medianBlur(grayMat, grayMat, 5);
	int moniter_range_width = grayMat.cols/7;
	cv::Rect mRect(grayMat.cols / 2 - moniter_range_width / 2,0,
		moniter_range_width, grayMat.rows);
	CMultipleCameraDlg*pDlg = ((CMultipleCameraDlg*)pParent);
	if (pDlg->preMat[nUsingDeviceNum].empty())
	{
		pDlg->preMat[nUsingDeviceNum] = grayMat(mRect).clone();
		return;
	}
	cv::Mat diffmat;
	cv::absdiff(pDlg->preMat[nUsingDeviceNum], grayMat(mRect), diffmat);
	pDlg->preMat[nUsingDeviceNum] = grayMat(mRect).clone();
	//cv::imshow(std::to_string(nUsingDeviceNum)+"diff", diffmat);


	//((CMultipleCameraDlg*)pParent)->m_saveNum_eidt1 += 1;
	//((CMultipleCameraDlg*)pParent)->UpdateData(FALSE);
	int grid_heigth = diffmat.rows / 6;
	std::vector<double> avgp_vec;
	for (int i = 0; i < 6; i++)
	{
		cv::Rect r;
		r.y += i * grid_heigth;
		r.height = grid_heigth;
		r.width = diffmat.cols;
		cv::Scalar m = cv::mean(diffmat(r));
		double b = m[0];
		avgp_vec.push_back(b);
	}
	std::sort(avgp_vec.begin(), avgp_vec.end());
	double max_diff = avgp_vec.back();
	
	if (max_diff > 30.0)
	{
		//std::string save_file_path = std::string("d:/") + std::to_string(nUsingDeviceNum) + "/";
		std::string save_img_path = CommonFunc::get_exe_dir();
		save_img_path = CommonFunc::joinFilePath(save_img_path, "Cam" + std::to_string(nUsingDeviceNum));
		//CString s = CommonFunc::MCharToWChar(save_img_path.c_str());
		save_img_path = save_img_path + "\\";
		MakeSureDirectoryPathExists(save_img_path.c_str());
		SYSTEMTIME sys;
		GetLocalTime(&sys);
		char timestr[256] = { 0 };
		sprintf(timestr, "IMG_%4d%02d%02d%02d%02d%02d%03d.jpg", sys.wYear, sys.wMonth, sys.wDay, sys.wHour, sys.wMinute, sys.wSecond, sys.wMilliseconds);
		save_img_path = CommonFunc::joinFilePath(save_img_path, timestr);
		OutputDebugString(CA2W((std::to_string(max_diff) + "\n").c_str()));
		OutputDebugString(CA2W((save_img_path + "\n").c_str()));
		cv::imwrite(save_img_path, srcImage);
		CEdit* pBoxOne;
		if(nUsingDeviceNum == 0)
		{
			pBoxOne = (CEdit*)GetDlgItem(IDC_SAVE_NUM_EDIT2);
		}
		else
		{
			pBoxOne = (CEdit*)GetDlgItem(IDC_SAVE_NUM_EDIT3);
		}
		pDlg->saved_img_num[nUsingDeviceNum]++;
		CString s;
		s.Format(_T("%d"), saved_img_num[nUsingDeviceNum]);
		pBoxOne->SetWindowText(s);

	}
	//cv::imshow(std::to_string(nUsingDeviceNum), resizedMat);
	//cv::waitKey(1);
	//cv::Mat srcm = cv::Mat(image_height, image_width, CV_8UC3, pImgData);
